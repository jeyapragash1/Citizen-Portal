...
def build_vector_index():
    ...
    if FAISS_AVAILABLE:
        try:
            ...
            try:
                ...
                try:
                    ...
                    pass
                grid_id = fs.put(idx_bytes, filename=f"faiss-{datetime.utcnow().isoformat()}.index")
                # store metadata pointer
                try:
                    db['vector_meta'].replace_one({"name": "current"}, {"name": "current", "meta_count": len(docs), "gridfs_id": grid_id, "updated": datetime.utcnow()}, upsert=True)
                except Exception:
                    pass
            except Exception as e:
        ...
        try:
            ...
            bio.seek(0)
            emb_grid_id = fs.put(bio.read(), filename=f"embeddings-{datetime.utcnow().isoformat()}.npy")
            # Persist metadata document list
            try:
                db['vector_meta'].replace_one({"name": "current"}, {"name": "current", "meta_count": len(docs), "embeddings_gridfs_id": emb_grid_id, "updated": datetime.utcnow(), "docs": docs}, upsert=True)
            except Exception:
                pass
        except Exception as e:
    ...
    try:
        db['vector_meta'].replace_one({"name": "current"}, {"name": "current", "meta_count": len(docs), "updated": datetime.utcnow(), "docs": docs}, upsert=True)
    except Exception as e:
...
def _run_build_index_background(job_id):
    ...
    try:
        ...
        INDEX_JOB_STATUS[job_id] = {"status": "running", "started_at": datetime.utcnow().isoformat()}
        try:
            index_jobs_col.update_one({"job_id": job_id}, {"$set": {"status": "running", "started_at": datetime.utcnow(), "updated_at": datetime.utcnow()}}, upsert=True)
            # append a lifecycle log entry
            try:
                index_jobs_col.update_one({"job_id": job_id}, {"$push": {"logs": {"ts": datetime.utcnow(), "msg": "job started"}}})
            except Exception:
                pass
        except Exception as e:
        ...
        INDEX_JOB_STATUS[job_id] = {"status": "completed", "finished_at": datetime.utcnow().isoformat(), "result": res}
        try:
            index_jobs_col.update_one({"job_id": job_id}, {"$set": {"status": "completed", "finished_at": datetime.utcnow(), "result": res, "updated_at": datetime.utcnow()}}, upsert=True)
            try:
                index_jobs_col.update_one({"job_id": job_id}, {"$push": {"logs": {"ts": datetime.utcnow(), "msg": f"job completed: {res.get('count', 0)} docs"}}})
            except Exception:
                pass
        except Exception as e:
        ...
        try:
            index_jobs_col.update_one({"job_id": job_id}, {"$set": {"status": "error", "error": str(e), "updated_at": datetime.utcnow()}}, upsert=True)
            try:
                index_jobs_col.update_one({"job_id": job_id}, {"$push": {"logs": {"ts": datetime.utcnow(), "msg": f"job error: {str(e)[:200]}"}}})
            except Exception:
                pass
...
def _run_fake_job_background(job_id):
    ...
    try:
        ...
        try:
            index_jobs_col.update_one({"job_id": job_id}, {"$set": {"status": "running", "started_at": datetime.utcnow(), "updated_at": datetime.utcnow()}}, upsert=True)
            try:
                index_jobs_col.update_one({"job_id": job_id}, {"$push": {"logs": {"ts": datetime.utcnow(), "msg": "fake job started"}}})
            except Exception:
                pass
        except Exception as e:
        ...
        INDEX_JOB_STATUS[job_id] = {"status": "completed", "finished_at": datetime.utcnow().isoformat(), "result": result}
        try:
            index_jobs_col.update_one({"job_id": job_id}, {"$set": {"status": "completed", "finished_at": datetime.utcnow(), "result": result, "updated_at": datetime.utcnow()}}, upsert=True)
            try:
                index_jobs_col.update_one({"job_id": job_id}, {"$push": {"logs": {"ts": datetime.utcnow(), "msg": "fake job completed"}}})
            except Exception:
                pass
        except Exception as e:
        ...
        try:
            index_jobs_col.update_one({"job_id": job_id}, {"$set": {"status": "error", "error": str(e), "updated_at": datetime.utcnow()}}, upsert=True)
            try:
                index_jobs_col.update_one({"job_id": job_id}, {"$push": {"logs": {"ts": datetime.utcnow(), "msg": f"fake job error: {str(e)[:200]}"}}})
            except Exception:
                pass
        except Exception as e2:
...
def admin_build_index_async():
    ...
    INDEX_JOB_STATUS[job_id] = {"status": "pending", "created_at": datetime.utcnow().isoformat()}
    # persist creation record
    try:
        index_jobs_col.update_one({"job_id": job_id}, {"$set": {"job_id": job_id, "status": "pending", "created_at": datetime.utcnow(), "updated_at": datetime.utcnow()}}, upsert=True)
    except Exception as e:
...
def admin_login():
    ...
    if admin:
        ...
        try:
            ...
            # Fallback for plain text passwords if stored password is not a hash (for dev convenience)
            # Ensure stored_password_hash is decoded for comparison if it's bytes
...
if __name__ == "__main__":
    ...
    if existing_admin:
        ...
        except Exception: # Broaden exception to catch any issue with old password format
            admins_col.update_one({"username": admin_username}, {"$set": {"password": hashed_password}})
